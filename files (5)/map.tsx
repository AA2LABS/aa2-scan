  ...
  const runRetailLoop = async () => {
    ...
    try {
      const response = await anthropic.messages.create({
        ...
        max_tokens: 1000,
        system: `You are The Chauffeur — AA2's retail intelligence engine. You fire INSIDE the store. Find what is better in this exact building right now. Cheaper. Better ingredients. Better nutrition. Calm, specific, never preachy.
Return ONLY valid JSON — no markdown, no backticks:
{"verdict":"string","betterOptions":[{"name":"string","why":"string","savings":"string or null"}],"chefNote":"string","storeSection":"string","actRightDollars":"string","equalizerNote":"string"}`,
        messages: [{
          role: 'user',
          content: `I am inside ${selectedStore.name} at ${selectedStore.vicinity}. I picked up: ${scannedItem}. What else in this store is cheaper, better ingredients, or better nutrition?`,
        }],
      });
  ...
  const buildSafeRoute = async () => {
    ...
    try {
      ...
      const response  = await anthropic.messages.create({
        ...
        messages: [{
          role: 'user',
          content: `Route: ${fullRoute}. Give a complete safety brief before I leave.`,
        }],
      });
  ...
  return (
    <SafeAreaView style={s.root}>
      ...
      {/* ── HEADER — CHAUFFEUR fix: paddingHorizontal on subtitle prevents clip ── */}
      <View style={s.header}>
        <Text style={s.headerTitle}>I AM THE RECEIPT</Text>
      ...
      <View style={s.toggleRow}>
        <TouchableOpacity
          style={[
            s.toggleBtn,
            mode === 'on' && { borderColor: GRID.on, backgroundColor: 'rgba(245,146,42,0.12)' },
          ]}
        ...
        <TouchableOpacity
          style={[
            s.toggleBtn,
            mode === 'off' && { borderColor: GRID.off, backgroundColor: 'rgba(46,207,179,0.12)' },
          ]}
          ...
          <>
            {hasOnResult && !loading ? (
              ...
              <View style={s.card}>
                <View style={s.cardTitleRow}>
                  <Text style={[s.gridBadge, { backgroundColor: GRID.on + '22', color: GRID.on, borderColor: GRID.on }]}>
                    ON GRID
                  </Text>
                  <Text style={s.cardTitle}>RETAIL INTELLIGENCE LOOP</Text>
                </View>
                <Text style={s.cardDesc}>
                  Fires inside the store. Finds what else in this building is cheaper, cleaner, or better nutrition.
                </Text>
                      ...
                      <TouchableOpacity
                        ...
                        style={[
                          ...
                          selectedStore?.name === store.name && {
                            borderColor: GRID.on,
                            backgroundColor: 'rgba(245,146,42,0.08)',
                          },
                        ]}
                ...
                <TouchableOpacity
                  style={[s.primaryBtn, { backgroundColor: GRID.on }]}
                  onPress={runRetailLoop}
                >
            ...
            {loading && (
              <View style={s.loadingCard}>
                ...
                <Text style={[s.loadingLabel, { color: GRID.on }]}>
                  THE CHAUFFEUR IS SCANNING THE STORE
                </Text>
              </View>
            )}
            ...
            {retailResult && !loading && (
              <View style={s.card}>
                ...
                {retailResult.storeSection && (
                  <Text style={s.storeSection}>📍 Find it: {retailResult.storeSection}</Text>
                )}
        ...
        {mode === 'off' && (
          <>
            {hasOffResult && !loading ? (
              ...
              <View style={s.card}>
                <View style={s.cardTitleRow}>
                  <Text style={[s.gridBadge, { backgroundColor: GRID.off + '22', color: GRID.off, borderColor: GRID.off }]}>
                    OFF GRID
                  </Text>
                  <Text style={s.cardTitle}>SAFETY TRAVEL ENGINE</Text>
                </View>
                <Text style={s.cardDesc}>
                  Pre-program your route before you leave. The Chauffeur briefs you — safe routes, exits, weather, news, emergency services, border notes.
                </Text>
                  ...
                  <>
                    ...
                    {waypoints.map((wp, i) => (
                      <View key={wp.id} style={s.waypointRow}>
                        ...
                        <TouchableOpacity
                          onPress={() => setWaypoints(prev => prev.filter(w => w.id !== wp.id))}
                        >
                ...
                <TouchableOpacity
                  style={[s.primaryBtn, { backgroundColor: GRID.off, marginTop: 14 }]}
                  onPress={buildSafeRoute}
                >
            ...
            {loading && (
              <View style={s.loadingCard}>
                ...
                <Text style={[s.loadingLabel, { color: GRID.off }]}>
                  THE CHAUFFEUR IS PLANNING YOUR ROUTE
                </Text>
                <Text style={s.loadingSubLabel}>
                  Domestic + International Safety Intelligence Active
                </Text>
              </View>
            )}
...
const s = StyleSheet.create({
  ...
  // ── Header — CHAUFFEUR fix ───────────────────────────────────────────────
  // Removed adjustsFontSizeToFit and numberOfLines from title to prevent
  // layout interference. Reduced letterSpacing on sub and added paddingHorizontal
  // so the trailing character of CHAUFFEUR is never clipped by parent bounds.
  header:        {
    alignItems: 'center',
    paddingTop: 10,
    paddingBottom: 10,
  ...
  headerSub:     {
    ...
    letterSpacing: 3,
    marginTop: 3,
    paddingHorizontal: 8,   // prevents trailing-letterSpacing clip on iOS
  },
